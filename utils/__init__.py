...
def prepare_for_result(cfg: Config):
    ...
    with open(output_path / 'logs/train.log', 'w') as fp:
        if cfg.experiment.mix_transform_strong:
            fp.write(
                    'Epochs\tlr\ttrain_loss\tcla_loss\tcell_loss\timg_loss\tp_loss\tcon_loss\tcell_loss_con\t'
                    'img_loss_con\tp_loss_con\n')
        else:
            fp.write('Epochs\tlr\ttrain_loss\tcell_loss\timg_loss\tp_loss\n')
    return output_path
...
def parse_args(mode='sp'):
    ...
    arg('mode', default='train', choices=['train', 'validate', 'predict_valid', 'predict_test', 'valid'])
